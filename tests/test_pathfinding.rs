use lazarust::{basemap::BaseMap, distance::DistanceAlg, pathfinding::a_star_search, point::Point};
...
fn a_star_with_diagonals() {
    ...
    assert_eq!(Point::new(1, 3), path[0]);
    assert_eq!(Point::new(2, 4), path[1]);
    assert_eq!(Point::new(3, 3), path[2]);
    assert_eq!(Point::new(3, 2), path[3]);
    assert_eq!(Point::new(2, 1), path[4]);
    assert_eq!(Point::new(1, 0), path[5]);
    assert_eq!(Point::new(0, 0), path[6]);
}
